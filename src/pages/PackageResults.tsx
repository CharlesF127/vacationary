...
const PackageResults = () => {
  ...
  const handleSetAlert = (packageItem: PackageOption) => {
    createPriceAlert({
      userId: 'user-1',
      type: 'package',
      origin: packageItem.origin,
      destination: packageItem.destination,
      maxPrice: packageItem.price - 50,
    });

    toast.success('Price alert created!', {
      description: `We'll notify you when packages to ${packageItem.destination} drop below $${packageItem.price - 50}.`
    });
  };
  ...
  const toggleDuration = (duration: string) => {
    ...
    updated.has(duration) ? updated.delete(duration) : updated.add(duration);
  ...
  const getAllDurations = () => {
    const durationsSet = new Set<string>();
    packages.forEach((pkg) => durationsSet.add(pkg.duration));
    return Array.from(durationsSet);
  };
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-2xl font-bold">
            Vacation Packages: {origin} to {destination}
          </h1>
        </div>
              ...
              <div className="space-y-2">
                {getAllDurations().sort().map((d) => (
                  <div key={d} className="flex items-center">
                    ...
                    />
                    <label htmlFor={`duration-${d}`} className="ml-2 text-sm">
                      {d}
                    </label>
                  </div>
                ))}
